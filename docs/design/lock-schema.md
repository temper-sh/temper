# `manifest.lock.yaml` schema design

Rewritten 2026-08-14 after owner review ("insanely overengineered" —
correct). The first draft accreted qualification and production-mode structure
— a profiles catalogue,
...
2. Record the exact pins so "am I on a tested configuration?" is
   ...
   tested versions (the software-supply catalog), and `check` compares the lock's pins
...
- `update [id]` now rewrites the whole row when its identity moves, prints the
  ...
  exists it also says when the new pin leaves the active catalog's tested
  set.
- `check` — now reads and reports repo/selection mismatch, missing rows,
  ...
  hash checks of selected files. With the software-supply catalog it also
  reports tested-set membership
  per entry.

There is no `attest` verb and no local verified state. The attestation decision
closed on 2026-08-18: tested-status is derived by comparing pins against the
...
- **Tested-status in the lock** → never. Owner ruling 2026-08-18:
  ...
  versions (the software-supply catalog);
  ...
  under the closed attestation decision.
...
- **Machine identity block** → with the consumer-home data-root work, when a lock synced
  between machines first becomes possible.
- **Profile citations** → with the qualification catalog. Agreed shape: a
...
- **Tool entries** → with qualification tool profiles, as their own section
