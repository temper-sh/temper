...
transaction. Signing/channel storage, real Homebrew/uv candidate readers,
tested-status reporting, and the public command surface remain pending. Later
schema changes still require review.

The first implementation slice needs to answer one narrow question:
...
Catalog update is explicit; there is no background updater. The future update
effect downloads or accepts a supplied snapshot, verifies signature, exact
...
1. **Package** — the thing Temper needs (`llama-swap`, `rapid-mlx`).
...
3. PEP 440 resolution pins `rapid-mlx` and the constrained exact `mlx` unit.
...
Real Homebrew/`uv` resolution, network reads, installation, and Field Kit runs
...
declared-but-unbuilt and descriptor mismatch refusals, structural closure/hash
validation, and canonical digests from fixtures 5 and 7–9. Candidate/version
selection (1–4 and 6) and the resolution transaction (10) are the next
executable slice.
