...
4. Extend the index loader only after each referenced typed document surface
   ...
   and model runtimes. Public evidence is accepted only with a recomputed typed
   scope; qualified profiles, other profile kinds, and recommendations fail
   closed.
